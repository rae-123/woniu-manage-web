  ...
  <v-card>
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-center"><img :src="props.item.imgurl " width="50px"></td>
        <td class="justify-center layout px-0">
          <v-btn icon small @click="editItem(props.item)">
            <i class="el-icon-edit"/>
          </v-btn>
          <v-btn icon small @click="deleteItem(props.item.productid)">
  ...
  export default {
    ...
    data() {
      return {
        ...
        pagination: {},// 分页信息
        headers: [// 表头
          {text: 'id', align: 'center', value: 'productid'},
          {text: '名称', align: 'center', sortable: false, value: 'name'},
          {text: '作者', align: 'center', sortable: false, value: 'author'},
          {text: '价格', align: 'center', value: 'price', sortable: true,},
          {text: '出版社', align: 'center', value: 'publisher', sortable: false,},
          {text: '类型', align: 'center', value: 'typeName', sortable: true,},
          {text: '图片', align: 'center', value: 'imgurl', sortable: true,},
    ...
    methods: {
      ...
      deleteItem(id) {
        this.$message.confirm('此操作将永久删除该商品, 是否继续?')
          .then(() => {
            // 发起删除请求
            this.$http.delete("/item/product/delete?id=" + id)
              .then(() => {
                // 删除成功，重新加载数据
      ...
      getDataFromApi() {
        this.loading = true;
        this.$http.get("/item/product/list")
          .then(resp => {
            console.log(resp);
